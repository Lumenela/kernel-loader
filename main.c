...
#define IMG_START_BLOCK 0
	...
	putc('\n');
	u32 hcrc = ((u32 *)LOAD_ADDRESS)[1];
	puts("\tHeader crc:\t\t");
	hcrc = ec(hcrc);
	dump_int((u32)hcrc);
	putc('\n');
	u32 header_size = 64;
	((u32 *)LOAD_ADDRESS)[1] = 0;
	u32 calc_header_crc = crc32((u32 *)LOAD_ADDRESS, header_size);
	puts("Calculated header CRC: "); 
	dump_int((u32)calc_header_crc);
	putc('\n');
	u32 dcrc = ((u32 *)LOAD_ADDRESS)[6];
	puts("\tData crc:\t\t");
	dcrc = ec(dcrc);
	dump_int((u32)dcrc);
	putc('\n');
...
//		WATCHDOG_RESET();
	...
	u32 calc_data_crc = crc32(load_addr, load_size);
	puts("Calculated CRC: ");
	dump_int((u32)calc_data_crc); 
	putc('\n');
